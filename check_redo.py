...
class CheckRedo:
        ...
        with open(self.redo_file) as file:
            ...
            for line in file:
                ...
                if re.findall(p_fl, line):
                    ...
                    continue
                if re.findall(p_zs, line):
                    self.zk_seq = (line[9:]).strip('\n').strip()
                ...
                for file in files:
                    if re.findall(p1, file):
                        try:
                            ...
                            logging.info("REDO: MOVE FILE:%s to %s" % (value + "/" + self.process_id + "/" + file, value + "/" + file))
    ...
    def do_task(self):
        ...
        没有redo文件，主程序正常往下执行
        回滚完毕，主程序正常往下执行
        入口文件没有处理完，需要重新处理，此时不需要再从源目录find文件，redo中需要记录seq
        """
        flag = 0
        action_step = self.read_redo()
        if action_step == -1:
            return flag, self.zk_seq, self.file_list
        if action_step == "BEGIN":
